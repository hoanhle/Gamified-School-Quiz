...
module.exports = {
    ...
    async listUsers(request, response) {
        const users = await User.find().sort('name').select('name email role').exec();
    ...
    async showUser(request, response) {
        ...
        if (!user) {
            request.flash('errorMessage', `User not found (id: ${request.params.id})`);
    ...
    async createUser(request, response) {
        ...
        await user.save();
        request.flash('successMessage', 'Registration ready, you can now log in.');
    ...
    async changeRole(request, response) {
        if (request.params.id === request.user.id) {
            request.flash('errorMessage', 'Changing your own role is not allowed.');
            return response.redirect('/users');
        }
    ...
    async processUpdateProfile(request, response) {
        ...
        if (request.user.email !== email) {
            ...
            if (user) {
                const errorMessage = 'This email has been registered for another user.';
        ...
        request.flash('successMessage', 'The information of this profile is updated successfully');
